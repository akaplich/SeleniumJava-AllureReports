...
class VerifyToastMessageforLinkingandUnlinkingPipeline{
    public void run(def params){
        WebElement toast
        
        try{
            toast = Elements.find(ID:"//*[contains(@class,'fractal-toast  success') or contains(@class, 'fractal-toast')]",Browser.Driver)
		}catch (StaleElementReferenceException e){
			System.out.println("Stale element encountered. Retrying...")
			sleep(500)
            toast = Elements.find(ID:"//*[contains(@class,'fractal-toast  success') or contains(@class, 'fractal-toast')]",Browser.Driver)
		}
        //WebElement toast = Elements.find(ID:"//*[contains(@class,'fractal-toast  success') or contains(@class, 'fractal-toast')]",Browser.Driver)
