...
class VerifyAlltableforActionItemspage{
    public void run(def params){

        if(params."Verify in Which List (optional)"){ActionItems.SwitchToItemList(List:params."Verify in Which List (optional)")}

        if(Exists.run(ID:"//*[@id='action_item_list_all' and @class='filter-btn active']")<=0){ 
            Click.run(ID:"//*[@id='action_item_list_all']")
            sleep(2000)
        }  

        VerifyText.run(ID:"//*[@id='action_item_all_count']","Text":params."All Tab Value")
