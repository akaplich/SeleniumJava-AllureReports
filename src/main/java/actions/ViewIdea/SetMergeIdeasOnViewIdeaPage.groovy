...
class SetMergeIdeasOnViewIdeaPage{
    public void run(def params){
        Click.run(ID:"//A[@id='inline-edit-icon-merged_ideas']", "Type of Click":"Javascript")
        if(params."Add or Remove"){
            params."Submission Title".split(",").eachWithIndex{ name, x ->
                if(params."Add or Remove".split(",")[x]=="Add"){
                    Click.run(ID:"//INPUT[@id='search-merge-ideas-input']")
                	SetText.run(ID:"//INPUT[@id='search-merge-ideas-input']", Text:name)
                    Click.run(ID:"//INPUT[@id='search-merge-ideas-input']/..//DIV[contains(@class,'search_users_button')]")
                    sleep(500)
