...
//C129825 - OLD RULE MODAL Added to Step - Idea Moved From Another Pipeline - Send Email
class C129825_OLDRULEMODALAddedtoStep_IdeaMovedFromAnotherPipeline_SendEmail
extends TestBase {
    ...
    @Test @Tag("OldRulesModalRegression")  public void testcase(){
        ...
        new actions.Mailinator.CreateMailinatorUser().run("Email":/oldbrightC129825/.toString(),"Enterprise System Administrator":true,"Finally Login as What User":/bi.adminuser1@brightidea.com/.toString())
        //Navigate
        ...
        new actions.Rules.SetRulePerformtheFollowingActions().run("Row Number to be Added, Delete or Modify existing":/1/.toString(),"Add, Delete or Modify":/Modify/.toString(),"Perform Following Action":/Send Email/.toString(),"Send Email Enter Subject":/Test Email/.toString(),"Send Email Text":/Rules Test Move Idea/.toString(),"Select Recipient":/System Admin Group/.toString())
        //OLD RULE Set Rule window
        new actions.Rules.SetRulewindow().run("Rule Name":/Added to Step - Send Email/.toString(),"Action":/Create/.toString())
        ...
        //Verify Email
        new actions.Mailinator.VerifyEmail().run("Email Address":/oldbrightC129825@mailinator.com/.toString(),"Subject":/Test Email/.toString(),"From":/noreply@brightidea.com/.toString(),"Full From":/noreply@brightidea.com <noreply@brightidea.com>/.toString())
    }
    //Basestate
