...
//C5088 - OLD RULE MODAL Clone Rule
class C5088_OLDRULEMODALCloneRule
extends TestBase {
    ...
    @Test @Tag("OldRulesModalRegression")  public void testcase(){
        ...
        new actions.Rules.SetRulewindow().run("Rule Name":/New Idea added to step/.toString(),"Action":/Create/.toString())
        //Select Rule in Rules Engine table
        new actions.Rules.SelectRuleinRulesEnginepage().run("Rule Name":/New Idea added to step/.toString())
        //OLD RULE Clone Rule
        new actions.Rules.CloneRule().run([:])
        //Verify Rule in Rules Engine table
        new actions.Rules.VerifyRuleinRulesEnginetable().run("Order":/2/.toString(),"Rule Name":/Clone of New Idea added to step/.toString(),"Event":/Added to Step Review/.toString(),"Last Run":/Never/.toString(),"Status":/Inactive/.toString(),"Number of Matches":/1/.toString())
    }
    //Basestate
