...
//C131753 - Send Tasks to WB > Stack > Status
class C131753_SendTaskstoWB_Stack_Status
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.SendIdeaToWhiteboardFromPipelineSteps().run("Existing Whiteboard or New Whiteboard":/New Whiteboard/.toString(),"Whiteboard Title":/StackNewStatus/.toString(),"Format":/Stack/.toString(),"Action":/Send/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/2/.toString())
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/User Homepage/.toString())
        ...
        new actions.Whiteboards.OpenExistingWhiteboardOnWhiteboardsPage().run("Name of Whiteboard":/StackNewStatus/.toString())
        ...
        //Verify Smart Matrix & Other Elements Exists On Whiteboard
        new actions.Whiteboards.VerifySmartMatrixExists().run("Verify This Actual Text":/Paused/.toString(),"Should Exist":true)
        //Go Back From Whiteboard
        new actions.Whiteboards.GoBackFromWhiteboard().run([:])
        //Create Whiteboard or Edit Template
        new actions.Whiteboards.CreateWhiteboardFromUserhome().run("Create New or Choose Template":/New Blank Whiteboard/.toString())
        //Set Whiteboard Title
        new actions.Whiteboards.EditTitleOfWhiteboard().run("Title":/StackStatusExisting/.toString(),"Action":/Submit/.toString())
        //Go Back From Whiteboard
        new actions.Whiteboards.GoBackFromWhiteboard().run([:])
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/User Homepage/.toString())
        ...
        new actions.ProjectRoom.SetActionsDropdowninTasksTab().run("Action Option":/Send to Whiteboard.../.toString())
        //Send Idea(s) to Whiteboard/Smart Matrix from Pipeline Steps/List View
        new actions.Whiteboards.SendIdeaToWhiteboardFromPipelineSteps().run("Existing Whiteboard or New Whiteboard":/Existing Whiteboard/.toString(),"Whiteboard Title":/StackStatusExisting/.toString(),"Format":/Stack/.toString(),"Add Assignee information":true,"Action":/Send/.toString())
        //Verify Toast Message in Tasks In Project Room
        new actions.ProjectRoom.VerifyToastMessageinTasksInProjectRoom().run("Message":/The selected tasks were sent successfully to the whiteboard: StackStatusExisting/.toString(),"Click on the link":true)
        ...
        //Compare Images
        new actions.selenium.CompareImages().run("Expected Image File":/C131753_expected.png/.toString(),"Actual Image Name":/C131753_actual.png/.toString(),"Crop Screenshot Start at X":/340/.toString(),"Crop Screenshot Start at Y":/450/.toString(),"Crop Screenshot Width":/215/.toString(),"Crop Screenshot Height":/215/.toString(),"Allowed Number of Different Pixels":/200/.toString())
    }
    //Basestate
    ...
    public void afterState(){
        ...
        try{
            //Delete File
            new actions.selenium.DeleteFile().run("File Location":/Downloads/.toString(),"File Name":/C131753_actual.png/.toString())
        }
       catch(all){}catch(Error err){}
        try{
            //Delete File
