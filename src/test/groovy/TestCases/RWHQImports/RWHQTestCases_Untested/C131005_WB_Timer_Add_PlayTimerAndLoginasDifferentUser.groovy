...
class C131005_WB_Timer_Add_PlayTimerAndLoginasDifferentUser
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.EditTitleOfWhiteboard().run("Title":/Timer/.toString(),"Action":/Submit/.toString())
        //Whiteboard - Set Share
        new actions.Whiteboards.SetShareWhiteboard().run("Click Share Button":true,"User(s)":/End User/.toString(),"User Add or Remove":/Add/.toString(),"Send Invite":true,"Action":/Close/.toString())
        //Copy Current URL
        ...
        new actions.Whiteboards.OpenExistingWhiteboardOnWhiteboardsPage().run("Tab":/Shared with Me/.toString(),"Name of Whiteboard":/Timer/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/30/.toString())
        //Verify Timer in Whiteboards
        new actions.Whiteboards.VerifyTimerWhiteboards().run("Timer Should be Present":true,"Timer Should be Running, Paused, Initialized or Minimilized":/running/.toString(),"Minutes Value":/01/.toString(),"Timer Set By":/System Admin/.toString())
    }
    //Basestate
