...
class C132300_DuplicatePipeline_IdeaPageTemplateAndCustomNav
{
    ...
    public void testcase(){
        ...
        new actions.ViewIdea30.SetTemplateSelection().run("Template":/Small Header (No Image)/.toString(),"Action":/Change Template/.toString())
        //Verify Idea on View Idea 30
        new actions.ViewIdea30.VerifyIdeaonViewIdea3().run("Template":/Small Header/.toString(),"Idea Name":/Hack Project/.toString())
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/Apps/.toString())
        //Create Duplicate Webstorm
        new actions.WebstormCreation.CreateDuplicateWebstorm().run("Pipeline To Duplicate":/Hack/.toString(),"Name":/Copy Hack Pipeline/.toString(),"Description":/Desc/.toString(),"Type of Licensing Model":/Standard Licensing Model/.toString(),"Action":/Duplicate/.toString(),"Should Admin Wait for Pipeline to be Created":true)
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Copy Hack Pipeline/.toString(),"Area to Navigate to":/Post Idea/.toString())
        //Set Custom and Other App Post Idea
        new actions.PostIdea.SetCustomAppPostIdea().run("Title":/Copy Hack Project/.toString(),"Submitter":/Team/.toString(),"Submitter Team User":/End User,Evaluator/.toString(),"Submitter Team User Add or Remove":/Add,Add/.toString(),"Team Submission Name":/Test123/.toString(),"Description":/Project One Desc/.toString(),"Category":/Theme 3/.toString(),"Action":/Submit Idea/.toString())
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Copy Hack Pipeline/.toString(),"Area to Navigate to":/Idea Board/.toString())
        //Access Idea in Idea Board page
        new actions.IdeaBoard.AccessIdeainIdeaBoardpage30().run("Idea Name":/Copy Hack Project/.toString())
        //Access or Close Idea from View Idea 3 Modal
        new actions.ViewIdea30.AccessIdeafromViewIdea3Modal().run("Access Idea":/Idea Title/.toString())
        //Verify Idea on View Idea 30
        new actions.ViewIdea30.VerifyIdeaonViewIdea3().run("Template":/Small Header/.toString(),"Idea Name":/Hack Project/.toString())
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Copy Hack Pipeline/.toString(),"Area to Navigate to":/Blog/.toString())
        //Set Blog Post
