...
//C131429 - MTS homepage > Memo Tab > All Filter for Shared with Link
class C131429_MTShomepage_MemoTab_AllFilterforSharedwithLink
{
    ...
    public void testcase(){
        ...
        //Copy Current URL
        variables."currentMemoURL1" = new actions.selenium.CopyCurrentURL().run([:])
        ...
        //Copy Current URL
        variables."currentMemoURL2" = new actions.selenium.CopyCurrentURL().run([:])
        //Collab Editor - Click Go Back Button
        new actions.Memo.ColabEditorClickGoBackButton().run([:])
        //Log Out
        ...
        //Verify Memo Card in Userhomepage
        new actions.Memo.VerifyMemoCardinUserhomepage().run("Memo Title":/Batched Shared Memo/.toString(),"Memo Icon Type":/Brightidea/.toString(),"User Name":/System Admin/.toString(),"Status":/Incubate 🐣/.toString(),"Thumbnail Image Displayed":true,"Number of Matches":/0/.toString())
        ...
        new actions.Memo.VerifyMemoCardinUserhomepage().run("Memo Title":/Batched Shared Memo/.toString(),"Memo Icon Type":/Brightidea/.toString(),"User Name":/System Admin/.toString(),"Status":/Incubate 🐣/.toString(),"Thumbnail Image Displayed":true,"Number of Matches":/0/.toString())
        //Navigate to URL
        new actions.selenium.NavigateToURL().run("URL":/${variables."currentMemoURL2"}/.toString())
        //Collab Editor - Click Go Back Button
        new actions.Memo.ColabEditorClickGoBackButton().run([:])
        //Navigate to URL
        new actions.selenium.NavigateToURL().run("URL":/${variables."currentMemoURL1"}/.toString())
        //Collab Editor - Click Go Back Button
        new actions.Memo.ColabEditorClickGoBackButton().run([:])
        //Set Filter for Memo in Userhomepage
