...
//C129667 - Move Ideas - Analyst Not Allowed
class C129667_MoveIdeas_AnalystNotAllowed
{
    ...
    public void testcase(){
        ...
        //Set Webstorm Setup Users Administrators page
        new actions.SetupUsers.SetWebstormSetupUsersAdministratorspage().run("WebStorm Administrators":/analyst/.toString(),"WebStorm Administrators Add or Remove":/Add/.toString())
        //Log Out
        new actions.general.LogOut().run([:])
        //Login
        new actions.general.Login().run("Email":/analyst@test.com/.toString(),"Password":/brightidea1/.toString())
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Solve App/.toString(),"Area to Navigate to":/Pipeline List/.toString())
        //Select Ideas in Pipeline List View
        new actions.PipelineList.SelectIdeasPipelineListView().run("Idea Title(s)":/solve test idea1,solve test idea2/.toString())
        try{
            //Set New Move Ideas in Pipeline Steps page
        ...
        //Verify Idea in Pipeline Steps page
        new actions.PipelineStepsView.VerifyIdeainPipelineStepsViewpage().run("Idea Name":/solve test idea1/.toString(),"Number of Matches":/1/.toString())
        //Verify Idea in Pipeline Steps page
        new actions.PipelineStepsView.VerifyIdeainPipelineStepsViewpage().run("Idea Name":/solve test idea2/.toString(),"Number of Matches":/1/.toString())
    }
    //Basestate
