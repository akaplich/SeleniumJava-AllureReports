...
//C131812 - Hack > Participants Page - Invite to Join Team - Multiple and Single Submissions, In-App Notification
class C131812_Hack_ParticipantsPage_InvitetoJoinTeam_MultipleandSingleSubmissions_In_AppNotification
{
    ...
    public void testcase(){
        ...
        new actions.SetupBI.SetEnterpriseSetupforBIOnlyBetatab().run("Invite user to join team flow":true)
        //Navigate
        ...
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Hack/.toString(),"Area to Navigate to":/Post Idea/.toString())
        //Set Custom and Other App Post Idea
        new actions.PostIdea.SetCustomAppPostIdea().run("Title":/system admin single submission/.toString(),"Description":/test/.toString(),"Category":/Theme 1/.toString(),"Action":/Submit Idea/.toString())
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Hack/.toString(),"Area to Navigate to":/Participants Page/.toString())
        ...
        new actions.Webstorm.SetInviteUserToTeamModalOnParticipantsPage().run("Select project":/system admin single submission/.toString(),"Action":/Submit/.toString())
        //Log Out
        new actions.general.LogOut().run([:])
        //Login
        ...
        new actions.Notifications.VerifyNotifications().run("Notification message":/System Admin has invited you to join their team, system admin single submission/.toString(),"Number of Matches":/1/.toString())
        //Verify Notifications
        new actions.Notifications.VerifyNotifications().run("Notification message":/End User has invited you to join their team, team submission with end user/.toString(),"Number of Matches":/1/.toString())
        //Access Notification from Global Navigation
        new actions.Notifications.AccessNotificationfromGlobalNavigation().run("Notification Message":/System Admin has invited you to join their team, system admin single submission/.toString())
        //Verify Idea on View Idea 30
        new actions.ViewIdea30.VerifyIdeaonViewIdea3().run("Template":/Medium Header/.toString(),"Idea Name":/system admin single submission/.toString())
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/Home/.toString())
        //Verify Notifications
        new actions.Notifications.VerifyNotifications().run("Notification message":/System Admin has invited you to join their team, system admin single submission/.toString(),"Notification Read Status":/Read/.toString(),"Number of Matches":/1/.toString())
        //Verify Notifications
        new actions.Notifications.VerifyNotifications().run("Notification message":/End User has invited you to join their team, team submission with end user/.toString(),"Number of Matches":/1/.toString())
        //Access Notification from Global Navigation
