...
//C129368 - Hide Private Comments - view comments as evaluator of 2 dev steps with different options on View Idea with new commenting
class C129368_HidePrivateComments_viewcommentsasevaluatorof2devstepswithdifferentoptionsonViewIdeawithnewcommenting
{
    ...
    public void testcase(){
        ...
        //Create Step in Pipeline Steps page
        new actions.PipelineStepsView.CreateStepinPipelineStepspage().run("Step Name":/New Development Step/.toString(),"Select Tool Type":/Development/.toString(),"Action":/Create/.toString())
        //Configure Development Step in Pipeline Steps page
        new actions.PipelineStepsConfigure.ConfigureDevelopmentinNewModalinPipelineStepspage().run("Assignment Method":/Category Assignment/.toString(),"Development Lead Category":/New Product,Operations,Marketing & Branding,Strategy,Quick Add\/New Categories/.toString(),"Development Lead":/Select a user,Select a user,Select a user,Select a user,Select a user/.toString(),"Selected user":/Evaluator,Evaluator,Evaluator,Evaluator,Evaluator/.toString(),"Selected Users Add or Remove":/Add,Add,Add,Add,Add/.toString(),"Development Field":/Development Long Answer Question/.toString(),"Development Field Included or Required":/Included/.toString(),"Action":/Activate/.toString())
        ...
        new actions.PipelineStepsView.SelectIdeainPipelineStepsViewpage().run("Idea Name":/test idea1/.toString())
        //Change Idea Step in Pipeline Steps page
        new actions.PipelineStepsView.ChangeIdeaStepinPipelineStepspage().run("Step":/New Development Step/.toString(),"Action":/Change Step/.toString())
        //Navigate WebStorm or MTS
        ...
        new actions.ViewIdeaComment.VerifyCommentinViewandNGAViewIdea().run("Comment section":/Parent Comment/.toString(),"Comment Type":/Private/.toString(),"Parent Comment":/test private comment1 by admin/.toString(),"User":/System Admin/.toString(),"Placement Number":/1/.toString())
        //Verify Comment in View and NGA View Idea
        ...
        //Verify Comment in View and NGA View Idea
        new actions.ViewIdeaComment.VerifyCommentinViewandNGAViewIdea().run("Comment section":/Parent Comment/.toString(),"Comment Type":/General/.toString(),"Parent Comment":/test comment/.toString(),"User":/End User/.toString(),"Placement Number":/1/.toString())
    }
    //Basestate
