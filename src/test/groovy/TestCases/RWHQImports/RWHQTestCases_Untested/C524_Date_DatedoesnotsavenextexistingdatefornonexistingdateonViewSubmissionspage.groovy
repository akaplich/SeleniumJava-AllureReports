...
//C524 - Date - Date does not save next existing date for nonexisting date on View Submissions page 
class C524_Date_DatedoesnotsavenextexistingdatefornonexistingdateonViewSubmissionspage
{
    ...
    public void testcase(){
        ...
        new actions.ViewIdeaFields.SetViewIdeaDescriptionTabTextField().run("Field Name":/Date Question/.toString(),"Field Value":/02\/31\/2016/.toString(),"Action":/Save/.toString())
        //Verify Error Field in View Idea page
        new actions.ViewIdeaFields.VerifyErrorRequiredFieldViewIdeaPage().run("Field Name":/Date Question/.toString(),"Should Error Exist":true)
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Custom App/.toString(),"Area to Navigate to":/Idea Board/.toString())
        //Access Idea in Idea Board page
        new actions.IdeaBoard.AccessIdeainIdeaBoardpage30().run("Idea Name":/test idea2/.toString())
        //Verify Custom App Description tab fields in View Idea page
        Action5812921cfa4ee77809f46b4f("Date Question":/10\/12\/2016/.toString())
    }
    //Basestate
