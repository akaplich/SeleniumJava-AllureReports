...
//C131332 - Memo - Collab Editor - System Admin Edit Others' Comments
class C131332_Memo_CollabEditor_SystemAdminEditOthers'Comments
{
    ...
    public void testcase(){
        ...
        new actions.Memo.CollabEditorAddComment().run("Comment":/comment from Evaluator for/.toString(),"Parent or Reply":/Parent/.toString(),"Action":/Comment/.toString(),"Tag Usernames":/System/.toString(),"Tag Email":/bi.adminuser1@brightidea.com/.toString(),"Assign to Username":/System Admin/.toString())
        //Collab Editor/Userhome - Memo Share Dialog -  Set Participants
        new actions.Memo.MemoShareDialogSetParticipants().run("Click Share Button":true,"User(s)":/System Admin/.toString(),"Permission (required)":/can edit/.toString(),"Send Invite":/TRUE/.toString(),"Action":/Close/.toString())
        ...
        new actions.Memo.SelectExistingMemosfromUserhome().run("Memo Title":/Comment Test/.toString())
        //Collab Editor - Verify Memo Comment
        new actions.Memo.CollabEditorVerifyMemoComment().run("Where to verify the comments?":/Comment Popover Next to Document/.toString(),"Comment to Verify":/comment from Evaluator for/.toString(),"Number of Matches":/1/.toString())
        try{
            //Collab Editor - Set Memo Comment Edit Menu
            new actions.Memo.SetMemoCommentEditMenu().run("Where to edit the comments?":/Comment Popover Next to Document/.toString(),"What Comment to Edit":/comment from Evaluator for/.toString(),"Menu Option":/Edit/.toString(),"Edited Comment":/edited comment/.toString())
        }
       catch(all){}catch(Error err){}
        //Collab Editor - Verify Memo Comment
        new actions.Memo.CollabEditorVerifyMemoComment().run("Where to verify the comments?":/Comment Popover Next to Document/.toString(),"Comment to Verify":/edited comment/.toString(),"Number of Matches":/1/.toString())
    }
    //Basestate
