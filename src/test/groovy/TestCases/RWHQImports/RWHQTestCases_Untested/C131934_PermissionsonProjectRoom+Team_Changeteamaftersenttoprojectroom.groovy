...
class C131934_PermissionsonProjectRoom+Team_Changeteamaftersenttoprojectroom
{
    ...
    public void testcase(){
        ...
        new actions.PostIdea.SetCustomAppPostIdea().run("Title":/end user idea/.toString(),"Description":/desc/.toString(),"Category":/New Product/.toString(),"Required Short Question":/short/.toString(),"Action":/Submit Idea/.toString())
        //Log Out
        new actions.general.LogOut().run([:])
        //Login
        ...
        new actions.ProjectRoom.SetProjectRoomTitle().run("Title":/Existing PR/.toString(),"Action":/Submit/.toString())
        //Navigate WebStorm or MTS
        ...
        new actions.PipelineTopIdeas.RightClickOnIdeaInTopIdeas().run("Idea Title":/end user idea/.toString())
        //Set Right Click Menu in Top Ideas
        new actions.PipelineTopIdeas.SetRightClickMenuInTopIdeas().run("Select Option":/Send to Project Room.../.toString())
        //Send to Project Room
        new actions.PipelineStepsView.SendToProjectRoom().run("From the Actions Menu":false,"Existing or New":/Existing/.toString(),"Title":/Existing PR/.toString(),"Action":/Submit/.toString(),"Wait":false)
        //Verify Toast Message in Tasks In Project Room
        new actions.ProjectRoom.VerifyToastMessageinTasksInProjectRoom().run("Message":/Your Idea has been added to your project room. Existing PR/.toString(),"Click on the link":false)
        //Navigate WebStorm or MTS
        ...
        new actions.ViewIdea.SetSubmitterinViewIdeaPage().run("User":/End User 2,analyst,Pipeline Admin,Moderator/.toString(),"Add or Remove or Set Owner":/Add,Add,Add,Add/.toString(),"Action":/Save/.toString())
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/User Homepage/.toString())
        //Navigate Tab User Homepage or MTS
        new actions.Enterprise.UserHomepage.NavigateTabUserHomepage().run("Tab":/Project Room/.toString())
        //Verify and Access Project Room Card in Userhome
        new actions.ProjectRoom.VerifyProjectRoomCard().run("Title":/Existing PR/.toString(),"Member count":/6/.toString(),"Number of Matches":/1/.toString(),"Access Project Room":true)
