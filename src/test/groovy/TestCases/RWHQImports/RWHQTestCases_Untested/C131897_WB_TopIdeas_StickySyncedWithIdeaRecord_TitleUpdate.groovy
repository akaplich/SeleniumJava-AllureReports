...
class C131897_WB_TopIdeas_StickySyncedWithIdeaRecord_TitleUpdate
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.EditTitleOfWhiteboard().run("Title":/C131897/.toString(),"Action":/Submit/.toString())
        ...
        new actions.Whiteboards.SetRightClickonStickyNote().run("Right Click on Sticky Note":false,"Add to pipeline":/Custom App/.toString(),"Select Pipeline Category":/New Product/.toString(),"Add to pipeline Action":/Add/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/5/.toString())
        //Send Key(s) To Whiteboard
        new actions.Whiteboards.SendKeysToWhiteboard().run("Key(s) to send":/updated/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/5/.toString())
        //Click on Whiteboard Canvas
        ...
        new actions.Whiteboards.VerifySmartMatrixExists().run("Verify This Actual Text":/updatedidea1/.toString(),"Should Exist":true)
        //Go Back From Whiteboard
        new actions.Whiteboards.GoBackFromWhiteboard().run([:])
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Custom App/.toString(),"Area to Navigate to":/Idea Board/.toString())
        //Access Idea in Idea Board page
        new actions.IdeaBoard.AccessIdeainIdeaBoardpage30().run("Idea Name":/updatedidea1/.toString())
        //Verify Idea on View Idea 30
        new actions.ViewIdea30.VerifyIdeaonViewIdea3().run("Template":/Medium Header/.toString(),"Idea Name":/updatedidea1/.toString(),"Category":/New Product/.toString())
        ...
        new actions.Whiteboards.OpenExistingWhiteboardOnWhiteboardsPage().run("Name of Whiteboard":/C131897/.toString())
        //Verify Smart Matrix & Other Elements Exists On Whiteboard
        new actions.Whiteboards.VerifySmartMatrixExists().run("Verify This Actual Text":/new idea synced title/.toString(),"Should Exist":true)
    }
    //Basestate
