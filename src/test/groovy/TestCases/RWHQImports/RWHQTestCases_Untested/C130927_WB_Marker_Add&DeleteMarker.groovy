...
class C130927_WB_Marker_AddAndDeleteMarker
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.ClickOnWhiteboardCanvas().run("X coordinate":/-4/.toString(),"Y coordinate":/-4/.toString(),"Single Double or Right Click":/Single Click/.toString())
        //Send Key(s) To Whiteboard
        new actions.Whiteboards.SendKeysToWhiteboard().run("Key(s) to send":/Delete/.toString())
        //Verify Smart Matrix & Other Elements Exists On Whiteboard
        new actions.Whiteboards.VerifySmartMatrixExists().run("Element (skip if verifying text)":/Marker/.toString(),"Should Exist":false)
    }
    //Basestate
