...
//C131763 - Apply - Applicants Tab - Send Message, View Conversation Links
class C131763_Apply_ApplicantsTab_SendMessage_ViewConversationLinks
{
    ...
    public void testcase(){
        ...
        new actions.PipelineSetup.SetRolesinPipelineSetupPage().run("Type of Licensing Model":/Standard Licensing Model/.toString(),"Navigate to Tab":true,"Analysts":/analyst@test.com/.toString(),"Analysts Add or Remove":/Add/.toString())
        //Navigate Webstorm via Admin Dropdown Only
        ...
        new actions.general.SetMessageInMessagePopUpOnIdeaBoards().run("Message Body":/${variables."TCData.Message"}/.toString(),"Action":/Submit/.toString())
        //Access or Close Idea from View Idea 3 Modal
        new actions.ViewIdea30.AccessIdeafromViewIdea3Modal().run("Close Idea Modal":true)
        //Log Out
        new actions.general.LogOut().run([:])
        ...
        new actions.Messages.AccessMessagesinMessagesGlobalNavigation().run("Sender":/${variables."TCData.UserName"}/.toString(),"Message":/${variables."TCData.Message"}/.toString())
        //Set Actions on View Private Message
        new actions.Messages.SetActionsonViewPrivateMessage().run("Reply Message":/${variables."TCData.Reply"}/.toString())
        //Log Out
        new actions.general.LogOut().run([:])
        //Login
        new actions.general.Login().run("Email":/${variables."TCData.User"}/.toString(),"Password":/brightidea1/.toString())
        ...
        new actions.IdeaBoard.AccessIdeainIdeaBoardpage30().run("Idea Name":/test/.toString())
        //Accept/Decline Applicant, Send Message on View Idea 3
        new actions.ViewIdea30.AcceptDeclineApplicantSendMessageonViewIdea3().run("Applicant's Username":/End User 2/.toString(),"Accept or Reject or Send Message":/View conversation/.toString())
        //Verify Private Messages on View Private Message
        new actions.Messages.VerifyPrivateMessagesonViewPrivateMessage().run("Subject":/test/.toString(),"Sender":/End User 2/.toString(),"Message":/${variables."TCData.Reply"}/.toString(),"Number of Matches":/1/.toString())
    }
    //Basestate
