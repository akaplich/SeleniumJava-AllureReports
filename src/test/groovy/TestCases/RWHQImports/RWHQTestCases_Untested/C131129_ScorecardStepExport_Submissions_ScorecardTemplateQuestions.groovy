...
//C131129 - Scorecard Step Export - Submissions - Scorecard Template Questions
class C131129_ScorecardStepExport_Submissions_ScorecardTemplateQuestions
{
    ...
    public void testcase(){
        ...
        //Set Evaluation Scorecard for Idea in Action Items page
        new actions.ActionItems.SetEvaluationScorecardIdeaActionItemspage().run("Multi Choice One Answer - Question":/Export Question/.toString(),"Multi Choice One - Answer(s)":/export2/.toString(),"Submit Scorecard":true)
        //Log Out
        new actions.general.LogOut().run([:])
        //Login
        new actions.general.Login().run("Email":/bi.adminuser1@brightidea.com/.toString(),"Password":/brightidea1/.toString())
        //Create Mailinator User
        new actions.Mailinator.CreateMailinatorUser().run("Email":/bright131129/.toString(),"Enterprise System Administrator":true,"Finally Login as What User":/bright131129@mailinator.com/.toString())
        //Navigate WebStorm or MTS
        new actions.general.NavigateWebStormAdminBar().run("WebStorm Name":/Custom App/.toString(),"Area to Navigate to":/Pipeline Steps/.toString())
        ...
        variables."link" = new actions.Mailinator.GetDownloadLinkfromEmail().run("Email Address":/bright131129/.toString())
        //Navigate to URL
        new actions.selenium.NavigateToURL().run("URL":/${variables."link"}/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/5/.toString())
        //Verify File Data
        new actions.general.VerifyFileData().run("File Name":/Custom_App-Scorecarding_Idea*.xlsx/.toString(),"File Location":/Downloads/.toString(),"Row Values":["test idea1","13.0"],"Number of Matches":/1/.toString(),"Total Number of Rows":/3/.toString())
        //Navigate to URL
        new actions.selenium.NavigateToURL().run("URL":/${variables."link"}/.toString())
        //Wait
        new actions.general.Wait().run("Seconds":/5/.toString())
        //Verify File Data
        new actions.general.VerifyFileData().run("File Name":/Custom_App-Scorecarding_Idea*.xlsx/.toString(),"File Location":/Downloads/.toString(),"Row Values":["test idea2","13.0","12.0"],"Number of Matches":/1/.toString(),"Total Number of Rows":/3/.toString())
    }
    //Basestate
