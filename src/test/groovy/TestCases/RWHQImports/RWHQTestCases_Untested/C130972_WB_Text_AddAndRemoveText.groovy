...
class C130972_WB_Text_AddAndRemoveText
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.EditTitleOfWhiteboard().run("Title":/C130972/.toString(),"Action":/Submit/.toString())
        ...
        new actions.Whiteboards.SendKeysToWhiteboard().run("Key(s) to send":/tc130972/.toString())
        //Refresh
        new actions.selenium.Refresh().run([:])
        //Verify Smart Matrix & Other Elements Exists On Whiteboard
        new actions.Whiteboards.VerifySmartMatrixExists().run("Verify This Actual Text":/tc130972/.toString(),"Should Exist":true)
        //Click on Whiteboard Canvas
        new actions.Whiteboards.ClickOnWhiteboardCanvas().run("X coordinate":/10/.toString(),"Y coordinate":/5/.toString(),"Single Double or Right Click":/Single Click/.toString())
        //Send Key(s) To Whiteboard
        new actions.Whiteboards.SendKeysToWhiteboard().run("Key(s) to send":/Delete/.toString())
        //Verify Smart Matrix & Other Elements Exists On Whiteboard
        new actions.Whiteboards.VerifySmartMatrixExists().run("Element (skip if verifying text)":/Text 1/.toString(),"Should Exist":false)
    }
    //Basestate
