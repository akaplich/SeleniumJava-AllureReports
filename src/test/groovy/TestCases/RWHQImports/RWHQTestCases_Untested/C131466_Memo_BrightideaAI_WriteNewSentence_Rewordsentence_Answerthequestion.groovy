...
//C131466 - Memo - Brightidea AI - Write New Sentence, Reword sentence, Answer the question
class C131466_Memo_BrightideaAI_WriteNewSentence_Rewordsentence_Answerthequestion
{
    ...
    public void testcase(){
        ...
        new actions.Memo.ColabEditorSetText().run("Text":/Need a sentence with the word morning./.toString())
        //Collab Editor -  Set ToolBar Options
        new actions.Memo.CollabEditorSetToolBarOptions().run("Brightidea AI":/Write next sentence/.toString())
        //Collab Editor - Verify Text
        new actions.Memo.CollabEditorVerifyText().run("Contains Text (only verifies text & color!)":/morning/.toString(),"Font Color":/rgb(155,89,182)/.toString(),"Should Text Exist":true)
        //Collab Editor - Set Text
        new actions.Memo.ColabEditorSetText().run("Text":/Good ideas are always good ideas/.toString())
        //Collab Editor -  Set ToolBar Options
        new actions.Memo.CollabEditorSetToolBarOptions().run("Brightidea AI":/Reword sentence/.toString())
        try{
            //Collab Editor - Verify Text
            new actions.Memo.CollabEditorVerifyText().run("Contains Text (only verifies text & color!)":/day/.toString(),"Font Color":/rgb(155,89,182)/.toString(),"Should Text Exist":true)
        }
       catch(all){println all}catch(Error err){println err}
        //Collab Editor - Set Text
        new actions.Memo.ColabEditorSetText().run("Text":/How many Mondays are in a single week?/.toString())
        //Collab Editor -  Set ToolBar Options
        new actions.Memo.CollabEditorSetToolBarOptions().run("Brightidea AI":/Answer the question/.toString())
        //Collab Editor - Verify Text
        new actions.Memo.CollabEditorVerifyText().run("Contains Text (only verifies text & color!)":/one/.toString(),"Font Color":/rgb(155,89,182)/.toString(),"Should Text Exist":true)
    }
    //Basestate
