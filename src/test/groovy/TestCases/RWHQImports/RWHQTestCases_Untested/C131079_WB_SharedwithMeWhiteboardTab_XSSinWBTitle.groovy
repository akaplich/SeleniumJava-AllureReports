...
//C131079 - WB - Shared with Me Whiteboard Tab, XSS in WB Title
class C131079_WB_SharedwithMeWhiteboardTab_XSSinWBTitle
{
    ...
    public void testcase(){
        ...
        new actions.Whiteboards.CreateWhiteboardFromUserhome().run("Create New or Choose Template":/Assumptions Grid/.toString())
        //Set Whiteboard Title
        new actions.Whiteboards.EditTitleOfWhiteboard().run("Title":/whiteboard one<script>alert('XSS')<\/script>/.toString(),"Action":/Submit/.toString())
        ...
        new actions.Whiteboards.VerifyWhiteboard().run("Whiteboard Name":/whiteboard one<script>alert('XSS')<\/script>/.toString(),"Last Edited Displayed":true,"Number of Matches":/0/.toString())
        //Log Out
        ...
        new actions.Notifications.AccessNotificationfromGlobalNavigation().run("Notification Message":/You’ve been invited to collaborate on whiteboard one!/.toString(),"Notification Read Status":/Unread/.toString())
        //Verify Whiteboard Title
        new actions.Whiteboards.VerifyWhiteboardTitle().run("Title Starts With":/whiteboard one/.toString())
        //Navigate
        new actions.general.Navigate().run("Area to Navigate to":/User Homepage/.toString())
        //Navigate Tab User Homepage or MTS
        new actions.Enterprise.UserHomepage.NavigateTabUserHomepage().run("Tab":/Whiteboards/.toString())
        ...
        new actions.Whiteboards.SetTabinWhiteboardList().run("Tab":/Shared with Me/.toString())
        //Verify Whiteboard Card in Userhomepage
        new actions.Whiteboards.VerifyWhiteboard().run("Whiteboard Name":/whiteboard one/.toString(),"Last Edited Displayed":true,"Number of Matches":/1/.toString())
        //Navigate to URL
