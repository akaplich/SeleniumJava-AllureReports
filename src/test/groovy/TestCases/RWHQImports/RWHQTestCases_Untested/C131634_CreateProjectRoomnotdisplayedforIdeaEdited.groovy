...
//C131634 - Create Project Room not displayed for Idea Edited
class C131634_CreateProjectRoomnotdisplayedforIdeaEdited
{
    ...
    public void testcase(){
        ...
        new actions.Rules.VerifyErrorFieldinRulesModal().run("Number of Error Fields":/1/.toString())
        //Set Rule Event that triggers this rule
        new actions.Rules.SetNewRuleEventthattriggersthisrule().run("Event":/Comment Submitted/.toString())
        try{
            //Set Rule Perform the Following Actions
            new actions.Rules.SetNewRulePerformtheFollowingActions().run("Row Number to be Added, Delete or Modify existing":/1/.toString(),"Add, Delete or Modify":/Modify/.toString(),"Perform Following Action":/Create new Project Room/.toString())
        }
       catch(all){}catch(Error err){}
        //Set Rule window
        ...
        new actions.Rules.VerifyErrorFieldinRulesModal().run("Number of Error Fields":/1/.toString())
        //Set Rule Event that triggers this rule
        new actions.Rules.SetNewRuleEventthattriggersthisrule().run("Event":/Removed from Step/.toString(),"Event Value":/Implementation/.toString())
        try{
            //Set Rule Perform the Following Actions
            new actions.Rules.SetNewRulePerformtheFollowingActions().run("Row Number to be Added, Delete or Modify existing":/1/.toString(),"Add, Delete or Modify":/Modify/.toString(),"Perform Following Action":/Create new Project Room/.toString())
        }
