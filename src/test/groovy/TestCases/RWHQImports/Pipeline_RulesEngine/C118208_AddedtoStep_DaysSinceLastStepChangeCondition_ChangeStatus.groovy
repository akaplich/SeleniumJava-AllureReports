...
//C118208 - Added to Step - Days Since Last Step Change Condition - Change Status
class C118208_AddedtoStep_DaysSinceLastStepChangeCondition_ChangeStatus extends TestBase {
    private static def variables = [:]      
    private static final Logger logger = LoggerFactory.getLogger(C118208_AddedtoStep_DaysSinceLastStepChangeCondition_ChangeStatus.class);
    ...
    public void testcase(){     
        ...
        try{
            ...
            new actions.Rules.SetNewRuleMeettheFollowingConditions().run("Row Number to be Added, Delete or Modify existing":/1/.toString(),"Add, Delete or Modify":/Add/.toString(),"Condition":/Days Since Last Step Change/.toString(),"Operator value":/Is Less Than/.toString(),"Condition value":/1/.toString())
        }
        ...
        try{
            //Set Rule window
            new actions.Rules.SetNewRulewindow().run("Rule Name":/Add Idea and Change Status/.toString(),"Action":/Create/.toString())
        }
       catch(all){}catch(Error err){}
        try{
        ...
        new actions.ViewIdea.VerifyIdeainViewIdeapage().run("Idea Name":/Quick Add for Rule/.toString(),"Status":/Completed/.toString(),"Linked Submissions":[])
    }
    //Basestate
