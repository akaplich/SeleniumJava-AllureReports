...
//C129867 - Vote Submitted - Perform Multiple Actions - Copy Idea & Add Comment
class C129867_VoteSubmitted_PerformMultipleActions_CopyIdeaAndAddComment extends TestBase {
    private static def variables = [:]
    private static final Logger logger = LoggerFactory.getLogger(C129867_VoteSubmitted_PerformMultipleActions_CopyIdeaAndAddComment.class);
    ...
    public void testcase(){     
        ...
        new actions.Rules.SetNewRuleEventthattriggersthisrule().run("Event":/Vote Submitted/.toString())
        //Set Rule Perform the Following Actions
        new actions.Rules.SetNewRulePerformtheFollowingActions().run("Row Number to be Added, Delete or Modify existing":/1/.toString(),"Add, Delete or Modify":/Add/.toString(),"Perform Following Action":/Add Public Comment/.toString(),"Commenter":/Pipeline Admin/.toString(),"Comment":/Idea Copied from Custom App/.toString())
        //Set Rule Perform the Following Actions
        new actions.Rules.SetNewRulePerformtheFollowingActions().run("Row Number to be Added, Delete or Modify existing":/2/.toString(),"Add, Delete or Modify":/Add/.toString(),"Perform Following Action":/Copy Idea to Pipeline/.toString(),"Configure Action (Move or Copy to Pipeline Only)":true)
        //Configure Rule Action Copy Idea to Pipeline
        new actions.Rules.ConfigureNewRuleActionCopyIdeatoPipeline().run("Pipeline":/Pipeline A/.toString(),"Copy Comments":true,"Action":/Save/.toString())
        //Set Rule window
        new actions.Rules.SetNewRulewindow().run("Rule Name":/Rule Copy/.toString(),"Action":/Create/.toString())
        //Select Rule in Rules Engine table
        new actions.Rules.SelectRuleinRulesEnginepage().run("Rule Name":/Rule Copy/.toString())
        //Change Rule Status
        ...
        new actions.ViewIdeaComment.VerifyCommentinViewandNGAViewIdea().run("Comment section":/Parent Comment/.toString(),"Comment Type":/General/.toString(),"Parent Comment":/Idea Copied from Custom App/.toString(),"User":/Pipeline Admin/.toString(),"Number of Matches":/1/.toString())
    }
     //Basestate
